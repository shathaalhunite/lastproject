...
class WidgetApplication(FrameBase):
    ...
    sig_launch_action_requested = Signal(object, object, bool, object, object, object, object)
...
class ListItemApplication(ListWidgetItemBase):
    ...
    def ordered_widgets(self):
        """Return a list of the ordered widgets."""
        return [self.button_license, self.button_install, self.button_launch, self.button_options]
    ...
    def actions_menu_requested(self):
        ...
        install_action = create_action(self.widget, 'Install application', triggered=lambda: self.install_application())
        install_action.setEnabled(not self.installed)

        if self.non_conda:
            install_action.setDisabled(True)

        update_action = create_action(self.widget, 'Update application', triggered=lambda: self.update_application())
        ...
        remove_action = create_action(self.widget, 'Remove application', triggered=lambda: self.remove_application())
    ...
    def update_status(self):
        ...
        if (self.versions and self.version != self.versions[-1] and self.installed):
            ...
            self.button_version.setToolTip('Version {0} available'.format(self.versions[-1]))
        else:
    ...
    def launch_application(self):
        ...
        if self.app_type == AppType.WEB:
            external_apps.web_apps[self.name](config=self.api.config).launch()
        else:
            if self.app_type == AppType.INSTALLABLE:
                command = external_apps.apps[self.name](
            ...
            self.widget.sig_launch_action_requested.emit(
                self.name, command, True, self.prefix, C.TAB_HOME, self.non_conda, self.app_type
            )
