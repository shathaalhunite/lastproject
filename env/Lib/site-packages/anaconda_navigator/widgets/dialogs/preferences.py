...
from qtpy.QtWidgets import (QCheckBox, QGridLayout, QHBoxLayout, QLabel,
                            QLineEdit, QScrollArea, QTextEdit, QVBoxLayout,
                            QWidget)
import yaml
...
class SettingsDialog(DialogBase):
    ...
    def _restart_application():
        ...
        if WIN:
            QtCore.QProcess.startDetached(sys.argv[0])
        else:
            QtCore.QProcess.startDetached(sys.executable, sys.argv)


class NavigatorSettingsDialog(SettingsDialog):
    ...
    def _save(self, e):
        ...
        if missing_data:
            text = (
                'The saved data missed some sections or attributes '
                'and could cause the issues with working Navigator! '
                'Please fix.'
            )
...
class CondaSettingsDialog(SettingsDialog):
...
class PreferencesDialog(DialogBase):
    ...
    def __init__(self, config=CONF, environments=None, **kwargs):
        """Application preferences dialog."""
    ...
    def create_textbox(self, label, option, hint=None, check=None, info=None, placeholder=None):
    ...
    def create_combobox(self, label, option, check=None, hint=None, info=None):
        widget = ComboBoxBase()
    ...
    def setup(self):
        ...
        self.create_textbox(
            'Team Edition API domain',
            'team_edition_api_url',
            placeholder='http(s)://example.com',
            check=lambda base_url: api_url_checker(f'{base_url}/api/system' if base_url else None, allow_blank=True),
        )
        self.create_textbox(
...
most web browsers and mobile apps.'''
        ...
        info_message = "Directory path where {0} is installed on your machine. \n" \
                       "To see {0} on the home tab, you will also need to click \n" \
                       "the Refresh button on the home tab."
        if BITS_64:
            self.create_textbox('PyCharm CE path', 'pycharm_ce_path', info=info_message.format("PyCharm Community"))

            self.create_textbox(
                'PyCharm Pro path', 'pycharm_pro_path', info=info_message.format("PyCharm Professional")
            )

        self.create_textbox('VS Code path', 'vscode_path', info=info_message.format("Visual Studio Code"))
