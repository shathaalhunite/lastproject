...
def botchannels(qtbot, tmpfile):
  ...
  - sean"""
    ...
    config_data = {'config_sources': {tmpfile: {'channels': ['chdoig', 'sean']}}}
...
def test_add_defaults(botchannels):
    ...
    with qtbot.waitSignal(signal=widget.sig_check_ready, timeout=5000, raising=True):
        item = widget.list.item(widget.list.count() - 1)
...
def test_add_valid_channels(botchannels, tmpfolder):
    ...
    for channel, channel_text in channels.items():
        ...
        with qtbot.waitSignal(signal=widget.sig_check_ready, timeout=20000, raising=True):
...
def test_remove_button_focus_signal(botchannels):
    ...
    with qtbot.waitSignal(signal=widget.list.sig_focus_fixed, timeout=20000, raising=True):
...
def test_menu_copy(botchannels):
    ...
    def _triggered():
        with qtbot.waitSignal(signal=item.text_channel.sig_copied, timeout=2000, raising=True):
