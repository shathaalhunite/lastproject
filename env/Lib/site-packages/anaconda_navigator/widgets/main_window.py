...
from qtpy.QtWidgets import (QApplication, QHBoxLayout, QMainWindow,
                            QVBoxLayout, QWidget)
from conda_token.repo_config import token_remove
...
from anaconda_navigator.config import CHANNELS_PATH, CONF, DEFAULT_ANACONDA_MAIN_URL, DEFAULT_BRAND, MAC, WIN
...
from anaconda_navigator.widgets import (ButtonBase, ButtonLink, ButtonPrimary, FrameBase, LabelBase,
                                        QSvgWidget, SpacerHorizontal, QLabel)
...
from anaconda_navigator.widgets.dialogs.login import MainLoginDialog, TeamEditionAddChannelsPage
...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        if tab_environments:
            ...
            # Signals
            self.tab_environments.sig_channels_requested.connect(self.show_channels)
            self.tab_environments.sig_update_index_requested.connect(self.update_index)
            self.tab_environments.sig_item_selected.connect(self.select_environment)
            self.tab_environments.sig_create_requested.connect(self.show_create_environment)
            self.tab_environments.sig_clone_requested.connect(self.show_clone_environment)
            self.tab_environments.sig_import_requested.connect(self.show_import_environment)
            self.tab_environments.sig_remove_requested.connect(self.show_remove_environment)
            self.tab_environments.sig_packages_action_requested.connect(self.show_conda_packages_action)
            self.tab_environments.sig_ready.connect(lambda: self.set_busy_status(conda=False))
            self.tab_environments.sig_cancel_requested.connect(self.show_cancel_process)
            self.sig_logged_in.connect(lambda: self.tab_environments.sig_update_index_requested.emit('Login'))
            self.sig_logged_out.connect(lambda: self.tab_environments.sig_update_index_requested.emit('Logout'))
        # if tab_project:
        ...
        self._timer_environments.timeout.connect(self.check_for_new_environments)
        ...
        self.button_logged_text.linkActivated.connect(lambda activated_url: self.open_url(activated_url))
    ...
    def get_logged_api_label_url(self):
        logged_api_url, anaconda_api_url, enterprise_4_repo_api_url, team_edition_api_url, commercial_edition_url = (
            parse.urlsplit(self.config.get('main', config_option) or '') for config_option in (
                'logged_api_url', 'anaconda_api_url', 'enterprise_4_repo_api_url', 'team_edition_api_url',
                'commercial_edition_url'
            )
        )

        if logged_api_url.netloc:
            if logged_api_url == enterprise_4_repo_api_url:
                domain = get_domain_from_api_url(enterprise_4_repo_api_url)
                label = 'Enterprise Edition'
                return label, domain

            elif logged_api_url == team_edition_api_url:
                domain = parse.urljoin(team_edition_api_url.geturl(), '/channels/')
                label = 'Team Edition'
                return label, domain

            elif logged_api_url == commercial_edition_url:
                domain = commercial_edition_url.geturl()
                label = 'Commercial Edition'
                return label, domain

        domain = get_domain_from_api_url(anaconda_api_url)
        label = 'Anaconda.org'
    ...
    def update_login_status(self, user_data=None):
        ...
        def _update_tool_tip(worker, output, error):
            ...
            if output:
                self._anaconda_url = output.get('main_url', DEFAULT_ANACONDA_MAIN_URL)
                self._brand = output.get('brand', DEFAULT_BRAND)
            else:
                self._anaconda_url = DEFAULT_ANACONDA_MAIN_URL
                self._brand = DEFAULT_BRAND
        ...
        logged_api_url = self.config.get('main', 'logged_api_url')

        if logged_api_url or user_data:
            self.username = user_data.get('login', '') if user_data else self.username
            self.logged = True
        ...
        if self.logged:
            ...
            label, domain = self.get_logged_api_label_url()

            text = 'Connected to <a href="" style="color:#43B049;text-decoration:none">{}</a>'.format(label)
            if worker.username and domain:
                domain = "{}/".format(domain) if domain[-1] != '/' else domain
                user_url = parse.urljoin(domain, worker.username)
                text += ' as <a href="{}" style="color:#43B049;text-decoration:none">{}</a>'.format(
                    user_url, worker.username
                )

            self.button_logged_text.setText(text)
    ...
    def handle_login(self, logout=False):
        ...
        if self.logged or logout:
            QApplication.setOverrideCursor(Qt.WaitCursor)
            token_remove()
            self.api.remove_login_data()
            self.api.logout()
            self.logged = False
            self.sig_logged_out.emit()
            self.tracker.track_event('authenticate', 'logout', label=self.username)
            self.update_login_status()
            logger.debug(str((self.logged, self.username)))
        else:
            self.login_dialog = MainLoginDialog(self)
            self.login_dialog.finished.connect(self.dialog_finished)
    ...
    def dialog_finished(self, status):
        if status:
            ...
            if self.tracker:
                self.tracker.track_event('authenticate', 'login', label=self.username)
        self._track_tab()
        self.update_login_status()
        logger.debug(str((self.logged, self.username)))
    ...
    def show_preferences(self):
        ...
        self._dialog_preferences = PreferencesDialog(parent=self, config=self.config, environments=self.environments)

        # If the api url was changed by the user, a logout is triggered
        self._dialog_preferences.sig_urls_updated.connect(lambda au, cu: self.handle_login(logout=True))
    ...
    def show_create_environment(self):
        ...
        logger.debug('')
        self.tracker.track_event('environments', 'create')
        self.tracker.track_page(
            '/environments/create',
        ...
        if dlg.exec_():
            ...
            if name and prefix:
                ...
                worker = self.api.create_environment(prefix=prefix, packages=pkgs)
    ...
    def show_import_environment(self):
        ...
        logger.debug('')
        self.tracker.track_page('/environments/import', pagetitle='Create new environment by import')
        ...
        if dlg.exec_():
            ...
            if selected_filter == dlg.CONDA_SPEC_FILES:
                ...
                if selected_filter == dlg.CONDA_ENV_FILES:
                    ...
                    ls = raw.split('\n')
                    deps = [line for line in ls if line and not line.strip().startswith('#')]
                    data = {'name': name, 'dependencies': ['python', {'pip': deps}]}
    ...
    def show_remove_environment(self):
        ...
        if prefix != self.api.ROOT_PREFIX:
            ...
            if dlg.exec_():
                ...
                if prefix == self.config.get('main', 'default_env'):
                    self.config.set('main', 'default_env', self.api.ROOT_PREFIX)
...
#                self.stop_refreshing_timer()
    ...
    def show_clone_environment(self):
        ...
        self.tracker.track_page('/environments/clone', pagetitle='Clone environment dialog')
        ...
        if dlg.exec_():
            ...
            if name and prefix:
                ...
                self.set_busy_status(conda=True)
                worker = self.api.clone_environment(clone_from_prefix=clone_from_prefix, prefix=prefix)
                worker.sig_partial.connect(self._conda_partial_output_ready)
                worker.sig_finished.connect(self._conda_output_ready)
    ...
    def show_conda_packages_action(self, conda_packages_actions, pip_packages_actions):
        ...
        self.tracker.track_page('/environments/packages', pagetitle='Packages actions dialog')
        self.set_busy_status(conda=True)
        if install_packages:
            pkgs = install_packages
            dlg = PackagesDialog(parent=self, packages=pkgs)
            worker_deps = self.api.install_packages(prefix=self.current_prefix, pkgs=pkgs, dry_run=True)
        elif update_packages:
            pkgs = update_packages
            dlg = PackagesDialog(parent=self, packages=pkgs, update_only=True)
            worker_deps = self.api.update_packages(prefix=self.current_prefix, pkgs=pkgs, dry_run=True)
            ...
            worker_deps = self.api.remove_packages(prefix=self.current_prefix, pkgs=pkgs, dry_run=True)
        self._dialog_environment_action = dlg
        ...
        if dlg.exec_():
            ...
            if remove_packages:
                ...
                worker = self.api.install_packages(prefix=self.current_prefix, pkgs=pkgs)
            elif update_packages:
                worker = self.api.update_packages(prefix=self.current_prefix, pkgs=pkgs)
    ...
    def update_status(self, action=None, message=None, value=None, max_value=None):
        ...
        for tab in [self.tab_home, self.tab_environments]:
            if tab:
                tab.update_status(action=action, message=message, value=value, max_value=max_value)
