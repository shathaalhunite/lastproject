...
class ClientWorker(QObject):
    ...
    def start(self):
        ...
        try:
            ...
            logger.debug(str((self.method.__module__, self.method.__name__, err)), )
            error = str(err)
            error = error.replace('(', '')
            error = error.replace(')', '')
...
class _ClientAPI(QObject):
    ...
    def _load_repodata(repodata, metadata=None, python_version=None):
        ...
        for name in all_packages:
            versions = tuple(sorted(all_packages[name]['versions'], reverse=True))
    ...
    def _prepare_model_data(packages, linked, pip=None):
        ...
        for name in packages_names:
            ...
            summary = p_data.get('summary') or '' if p_data else ''
            url = p_data.get('home') or '' if p_data else ''
            license_ = p_data.get('license') or '' if p_data else ''
            versions = p_data.get('versions') or '' if p_data else []
            version = p_data.get('latest_version') or '' if p_data else ''
    ...
    def reload_client(self):
        ...
        """
        logged_api_url = self.config.get('main', 'logged_api_url', None)
        anaconda_api_url = self.config.get('main', 'anaconda_api_url', None)
        team_edition_api_url = self.config.get('main', 'team_edition_api_url', None)
        enterprise_4_repo_api_url = self.config.get('main', 'enterprise_4_repo_api_url', None)

        if logged_api_url and logged_api_url in [anaconda_api_url, enterprise_4_repo_api_url]:
            return self._load_binstar_client(logged_api_url)
        elif logged_api_url and logged_api_url == team_edition_api_url:
            ...
            except (Unauthorized, SSLError) as e:
                logger.debug(str(e))
    ...
    def _load_team_edition_client(self, api_url):
        """
        Sets the '_anaconda_client_api' to user TeamEdition API insted of
    ...
    def _login(self, username, password, application, application_url):
        """Login callback."""
        if isinstance(self._anaconda_client_api, TeamEditionAPI):
            new_token = self._anaconda_client_api.authenticate(username, password)
    ...
    def login(self, username, password, application, application_url):
        ...
        method = self._login
        return self._create_worker(method, username, password, application, application_url)
    ...
    def logout(self):
        ...
        if isinstance(self._anaconda_client_api, TeamEditionAPI):
            ...
            if self.token:
                try:
                    ...
                    error = e
                    logger.debug("The token that you are trying to remove may " "not be valid {}".format(e))
        ...
        self.config.set('main', 'logged_api_url', None)
        return error
    ...
    def packages(self, login=None, platform=None, package_type=None, type_=None, access=None):
    ...
    def get_api_info_url(self):
        ...
        if isinstance(self._anaconda_client_api, TeamEditionAPI):
            return "{0}/api/system".format(self.config.get('main', 'team_edition_api_url'))
        else:
            config_data = binstar_client.utils.get_config()
            return config_data.get('url', 'https://api.anaconda.org')
    ...
    def get_ssl(self, set_conda_ssl=True):
        ...
        if set_conda_ssl:
            ssl_verify = self._conda_api.config_get('ssl_verify').communicate()[0].get('ssl_verify')
