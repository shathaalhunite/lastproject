...
class _AnacondaAPI(QObject):
    ...
    """
    TE_CONFIGURED = 'configured'
    TE_TO_CONFIGURE = 'to_configure'
    ...
    def login(self, username, password):
        ...
        """
        logger.debug('username: {}, password: {}'.format(username, '*' * len(password)))
    ...
    def process_packages(self, packages, prefix=None, blacklist=()):
        ...
        logger.debug('prefix: {}, packages: {}, blacklist={}'.format(prefix, packages, blacklist))
        ...
        def _pip_data_ready(worker, output, error):
            ...
            worker = self._client_api.prepare_model_data(clean_packages, linked, pip_packages)
            worker.base_worker = base_worker
    ...
    def process_apps(self, apps, prefix=None):
        ...
        APPS_DESCRIPTIONS = {
            ...
            'console_shortcut': ('Run a cmd.exe terminal with your current environment from '
                                 'Navigator activated'),
            'powershell_shortcut':
        ...
        for app_name in apps:
            ...
            description = APPS_DESCRIPTIONS.get(app_name, data.get('description', ''))

            version = versions[-1]  # Versions are sorted from small to big
            image_path = image_paths.get(app_name, images.ANACONDA_ICON_256_PATH)
            ...
            if app_entry:
                ...
                if 'ipython notebook' in app_entry.lower():
                    app_entry = app_entry.replace('ipython notebook', 'jupyter-notebook')
                elif 'ipython qtconsole' in app_entry.lower():
                    app_entry = app_entry.replace('ipython qtconsole', 'jupyter-qtconsole')

            needs_license = app_name.lower() in PACKAGES_WITH_LICENSE

            conda_pkg_version = self.conda_package_version(prefix=prefix, pkg=app_name, build=False)
            application = dict(
                name=app_name,
        ...
        for app in external_apps.web_apps.values():
            app = app(config=self._conda_api)
        ...
        for app in external_apps.apps.values():
            ...
            if ((app.non_conda and app.executable) or app.is_installation_enabled) and app.is_available:
                applications[app.app_name] = dict(
                    name=app.app_name,
                    display_name=app.display_name,
                    description=app.description,
    ...
    def clone_environment(self, clone_from_prefix, prefix):
        ...
        logger.debug('prefix: {}, clone_from_prefix: {}'.format(prefix, clone_from_prefix))
    ...
    def import_environment(self, prefix, file):
        ...
        worker = self._conda_api.create(prefix=prefix, file=file, offline=self.is_offline())
    ...
    def install_packages(self, prefix, pkgs, dry_run=False, no_default_python=False):
        ...
        logger.debug('prefix: {}, pkgs: {}, dry-run: {}'.format(prefix, pkgs, dry_run))
    ...
    def setup_commercial_edition_default_channels(self):
        commercial_edition_base_url = self.config.get('main', 'commercial_edition_url')
        default_channels = self._conda_api.load_rc().get('default_channels', tuple())
        if any(True for channel in default_channels if commercial_edition_base_url in channel):
            return
        configure_default_channels()

    def detect_commercial_edition_login(self):
        commercial_edition_base_url = self.config.get('main', 'commercial_edition_url')
        if any(True for token_domain in token_list().keys() if commercial_edition_base_url in token_domain):
            self.setup_commercial_edition_default_channels()
            self.config.set('main', 'logged_api_url', commercial_edition_base_url)

    # --- Team Edition Specific methods.
    # -------------------------------------------------------------------------
    def detect_team_edition_login(self):
        team_edition_api_url = self.config.get('main', 'team_edition_api_url')
        logged_api_url = self.config.get('main', 'logged_api_url')
        team_edition_token = self.config.get('main', 'team_edition_token')
        team_edition_token_id = self.config.get('main', 'team_edition_token_id')

        # We were logged through Navigator's login action.
        if all((logged_api_url, team_edition_token, team_edition_token_id)) and logged_api_url == team_edition_api_url:
            return self.TE_CONFIGURED

        # Try to check if token with team edition url (specified in anaconda-navigator.ini) is present.
        elif team_edition_api_url:
            tokens = token_list()

            for api_url in tokens:
                if team_edition_api_url in api_url and tokens[api_url]:
                    self.config.set('main', 'logged_api_url', team_edition_api_url)
                    self.config.set('main', 'team_edition_token', tokens[api_url])
                    return self.TE_TO_CONFIGURE

        # Team Edition API url wasn't specified in anaconda-navigator.ini
        # Check if user was logged in using Repo CLI.
        elif not team_edition_api_url:
            config = get_config()
            default_site_name = config.get('default_site')
            default_site_url = config.get('sites', {}).get(default_site_name, {}).get('url')
            ...
            if default_site_name and default_site_url and token:
                ...
                resulting_url = '{}://{}'.format(parsed_url.scheme, parsed_url.netloc)

                self.config.set('main', 'logged_api_url', resulting_url)
                self.config.set('main', 'team_edition_api_url', resulting_url)
                self.config.set('main', 'team_edition_token', token)
                return self.TE_TO_CONFIGURE

    def create_login_data(self, default_channels=None, channels=None):
        """
        ...
        if isinstance(self._client_api.anaconda_client_api, TeamEditionAPI):
            ...
            # Saving the copy of the data which was set before login action
            # To load it when user will do logout.
            self._conda_api.save_rc_copy(data=rc_data)

            rc_data.pop('default_channels', None)
            rc_data['channel_alias'] = '{}/api/repo'.format(self.config.get('main', 'team_edition_api_url'))
    ...
    def remove_login_data(self):
        ...
        if isinstance(self._client_api.anaconda_client_api, TeamEditionAPI):
            ...
            rc_data = self._conda_api.load_rc_copy()

            self._conda_api.save_rc(data=rc_data)
            self._conda_api.remove_rc_copy()
