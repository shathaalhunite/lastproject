...
def test_setup_commercial_edition_default_channels(api):
    config_default_channels_mock = mock.patch('anaconda_navigator.api.anaconda_api.configure_default_channels').start()

    with mock.patch.object(api._conda_api, 'load_rc'):
        api._conda_api.load_rc.return_value = {'default_channels': []}
        api.setup_commercial_edition_default_channels()
        config_default_channels_mock.assert_called()

        config_default_channels_mock.reset_mock()

        api._conda_api.load_rc.return_value = {
            'default_channels': [api.config.get('main', 'commercial_edition_url') + "/repo/test/path"]
        }
        api.setup_commercial_edition_default_channels()
        config_default_channels_mock.assert_not_called()


def test_detect_commercial_edition_login(api):
    commercial_edition_base_url = api.config.get('main', 'commercial_edition_url')
    token_list_mock = mock.patch('anaconda_navigator.api.anaconda_api.token_list').start()

    with mock.patch.object(api._conda_api, 'load_rc'), \
            mock.patch.object(api, 'setup_commercial_edition_default_channels'), mock.patch.object(api.config, 'set'):

        token_list_mock.return_value = {}
        api.detect_commercial_edition_login()
        api.setup_commercial_edition_default_channels.assert_not_called()
        api.config.set.assert_not_called()

        api.setup_commercial_edition_default_channels.reset_mock()
        api.config.set.reset_mock()

        token_list_mock.return_value = {commercial_edition_base_url: 'secret_ce_token'}
        api.detect_commercial_edition_login()
        api.setup_commercial_edition_default_channels.assert_called()
        api.config.set.assert_called_with('main', 'logged_api_url', commercial_edition_base_url)
