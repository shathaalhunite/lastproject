...
# Local imports
from anaconda_navigator.api.conda_api import _CondaAPI # analysis:ignore
...
def test_get_conda_version(qtbot, api):
    def worker_ready(worker, output, error):
        assert any(
            (
                output.startswith('4.3.'), output.startswith('4.4.'), output.startswith('4.5.'),
                output.startswith('4.6.'), output.startswith('4.7.'), output.startswith('4.8.'),
                output.startswith('4.9.'), output.startswith('4.10.'), output.startswith('4.11.')
            )
        )
...
def test_install_dry_keys(qtbot, api):
    ...
    def worker_ready(worker, output, error):
        ...
        pkgs = ['pylint']
        worker = api.install(prefix=worker.test_prefix, pkgs=pkgs, dry_run=True)
        worker.test_prefix = prefix
