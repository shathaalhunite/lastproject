...
class _DownloadAPI(QObject):
    ...
    def _download(
        ...
        if self._is_internet_available():
            ...
            logger.debug('Status code {0} - {1}'.format(status_code, url))

            if status_code != 200:
                logger.debug('Invalid url {0}'.format(url))
                return path
    ...
    def _is_valid_api_url(self, url, verify=None, allow_blank=False):
        ...
        if self._is_internet_available():
            try:
                ...
                logger.error(str(error))
        # data.get('status') == 'ok' is required for Team Edition check
    ...
    def is_valid_api_url(self, url, non_blocking=True, verify=None, allow_blank=False):
        ...
        if non_blocking:
            ...
            return self._create_worker(method, url, verify=verify, allow_blank=allow_blank)
        else:
