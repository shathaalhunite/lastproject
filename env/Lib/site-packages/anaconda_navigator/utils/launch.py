...
from anaconda_navigator.config import (CONF_PATH, LAUNCH_SCRIPTS_PATH, LINUX,
                                       MAC, WIN)
from anaconda_navigator.utils.logs import logger
from anaconda_navigator.app.main import is_debug_enabled
...
def get_scripts_path(root_prefix, prefix, default_scripts_path=LAUNCH_SCRIPTS_PATH):
    ...
    if root_prefix != prefix:
        scripts_path = os.path.join(default_scripts_path, prefix.split('/')[-1])
    return scripts_path
...
def remove_package_logs(root_prefix, prefix, default_scripts_path=LAUNCH_SCRIPTS_PATH):
    """Try to remove output, error logs for launched applications."""
    scripts_p = get_scripts_path(root_prefix, prefix, default_scripts_path=default_scripts_path)
...
def get_package_logs(
    ...
    if not is_debug_enabled():
        # use device null to redirect
        stdout_log_path = "nul" if sys.platform == "win32" else "/dev/null"
        stderr_log_path = "nul" if sys.platform == "win32" else "/dev/null"
        return stdout_log_path, stderr_log_path, id_

    scripts_path = get_scripts_path(root_prefix, prefix, default_scripts_path=default_scripts_path)
    ...
    if id_ is None:
        for i in range(1, 10000):
            stdout_log = "{package_name}-out-{i}.txt".format(package_name=package_name, i=i)
            stderr_log = "{package_name}-err-{i}.txt".format(package_name=package_name, i=i)
            if stdout_log not in files and stderr_log not in files:
                id_ = i
                break
    else:
        stdout_log = "{package_name}-out-{i}.txt".format(package_name=package_name, i=id_)
        stderr_log = "{package_name}-err-{i}.txt".format(package_name=package_name, i=id_)
...
def get_command_on_win(
    ...
    CREATE_NO_WINDOW = 0x0800200 # Create no window, Create new process group
    popen_dict = {
...
def py_in_console(activate=None, prog='python'):
    ...
    if not check_prog(prog, activate):
        raise RuntimeError('Program not available in environment: %s, %s', prog, activate)
    if prog == 'python':
        cmd = 'python -i'
    elif prog == 'ipython':
        cmd = 'ipython -i'
